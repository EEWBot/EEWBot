    ...
    public Mono<Void> execute(final EEWBot bot, final MessageCreateEvent event, final String lang) {
        return event.getMessage().getChannel()
                .flatMap(channel -> channel.createMessage(CommandUtils.createEmbed(lang)
                        ...
                        .addField("eewbot.cmd.time.field.lastpctime.name", bot.getExecutor().getTimeProvider().getLastComputerTime()
                                .map(ZonedDateTime::toString)
                                .orElse("eewbot.cmd.time.field.nonsync.value"), false)
                        .addField("eewbot.cmd.time.field.lastntptime.name", bot.getExecutor().getTimeProvider().getLastNTPTime()
                                .map(ZonedDateTime::toString)
                                .orElse("eewbot.cmd.time.field.nonsync.value"), false)
